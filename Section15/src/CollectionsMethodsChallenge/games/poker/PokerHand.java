...
public class PokerHand {
    ...
    public String toString() {
        return "%d %-16s Rank:%d %-40s %s".formatted(
            playerNo, score, score.ordinal(), hand, (!discards.isEmpty()) ? "Discards: " + discards : "");
    }
    ...
    private void setRank(int faceCount) {
        switch (faceCount) {
            ...
            case 3 -> {
                if (score == Ranking.NONE) score = Ranking.THREE_OF_A_KING;
                else score = Ranking.FULL_HOUSE;
            }
            case 2 -> {
                if (score == Ranking.NONE) score = Ranking.ONE_PAIR;
                else if (score == Ranking.THREE_OF_A_KING) score = Ranking.FULL_HOUSE;
